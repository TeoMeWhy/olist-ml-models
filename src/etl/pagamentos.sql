...
ON t1.idVendedor = t2.idVendedorWITH tb_join AS (
...
sum(case when descTipoPagamento = 'boleto' then vlPedidoMeioPagamento else 0 end) / sum(qtdePedidoMeioPagamento) as pct_qtd_boleto_pedido,
sum(case when descTipoPagamento = 'credit_card' then vlPedidoMeioPagamento else 0 end) / sum(qtdePedidoMeioPagamento) as pct_qtdit_card_pedido,
sum(case when descTipoPagamento = 'voucher' then vlPedidoMeioPagamento else 0 end)  / sum(qtdePedidoMeioPagamento) as pct_qtdvoucher_pedido,
sum(case when descTipoPagamento = 'debit_card' then vlPedidoMeioPagamento else 0 end) / sum(qtdePedidoMeioPagamento) as pct_qtdit_card_pedido,
...
SELECT idVendedor,
    AVG(nrParcelas) AS avgQtdeParcelas,
    PERCENTILE(nrParcelas, 0.5) AS medianaQtdeParcelas,
    MAX(nrParcelas) AS maxQtdeParcelas,
    MIN(nrParcelas) AS minQtdeParcelas,
